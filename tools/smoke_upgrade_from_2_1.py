#!/usr/bin/env python3
"""Create a published-version fixture, then prove 2.3.4b2 upgrades it safely."""
...
def create_fixture(root: Path) -> None:
    ...
    assert manifest["source_version"] in {
        "2.1.0", "2.2.3", "2.2.4", "2.2.5", "2.2.6", "2.3.2", "2.3.4b1"
    }
...
def verify_upgrade(root: Path) -> None:
    ...
    assert importlib.metadata.version("atmem") == "2.3.4b2"
    assert importlib.metadata.version("atmem-atbot") == "0.1.0"
    assert importlib.metadata.version("atflows") == "0.1b6"
    ...
    print("AtMem persisted-state -> 2.3.4b2 upgrade smoke test passed")
