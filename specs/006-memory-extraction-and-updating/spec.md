...
**Required, not yet implemented:** [Product requirements](../product-requirements.md) PR-002–PR-004. This amendment applies to this feature's public and UI boundaries; host-specific integrations cannot redefine core identity or authority.

- **FR-014**: Require one explicitly authorized destination memory space for admission or proposals and retain original contributor, authenticated actor, source and membership/policy revision. Shared read permission cannot authorize writes, promotion or sharing; cross-space duplicates/conflicts retain lineage and existing scopes keep their upgrade behavior.
- **SC-006**: Private/shared write fixtures reject unauthorized members and read-only writers, preserve contributor lineage under concurrent proposals, and do not import or reclassify legacy memory on upgrade.
