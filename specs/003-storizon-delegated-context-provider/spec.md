# Feature Specification: Storizon Delegated Context Provider
...
AtMem normally retrieves, authorizes, and prepares context itself. That behavior must remain unchanged for every existing installation and upgrade. Some integrations instead need an external system such as Storizon to remain the context authority. In delegated mode, a locally trusted provider returns one signed `inject` or `withhold` decision bound to the exact host turn. AtMem validates the result, reserves the turn atomically, injects accepted bytes without modification, suppresses its native context preparation for that turn, and records provider authorization separately from delivery.

The wire contract is provider-neutral. Storizon is the first supported provider but receives no privileged product-specific bypass. A public key sent inside a result is never trusted. Delegated mode cannot be activated until an operator registers a provider identity, key, allowed scope, transport endpoint, and failure behavior.
...
As an existing AtMem user, I can install the beta and continue using native AtMem context authorization without configuring Storizon or seeing new failures.
...
### User Story 2 — An operator safely enables Storizon authority (P1)

As an operator, I can register Storizon's identity and Ed25519 public key, restrict its scope, verify connectivity, and explicitly enable delegated mode with clear warnings about the changed authority boundary.
...
As a host user, when Storizon authorizes context for my exact turn, the host receives those exact bytes once and AtMem does not add a second memory context.
...
As a host user, a valid Storizon withholding decision or failed delegation never causes unintended native memory injection.
...
As an auditor, I can distinguish what Storizon authorized from what AtMem delivered and what the host/model later did.
...
3. Dashboard and CLI use plain language—“Storizon authorized” and “AtMem delivered”—while technical IDs remain available on demand.
...
- **FR-015**: The implementation MUST expose provider-neutral Python/control contracts so later Pydantic AI, LangGraph, Hermes, and other adapters can integrate without Storizon-specific core logic.
...
- **SC-006**: Clean-install and 2.2.5-upgrade tests pass on supported Python versions without requiring Storizon or a hosted model.
...
- Giving Storizon access to AtMem internals or giving AtMem access to Storizon's memory store.
