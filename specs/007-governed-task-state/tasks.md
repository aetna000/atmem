...
- [ ] [T007] Add `governed_task_profiles`, `governed_tasks` with nullable `paused_at_utc` and non-negative integer `no_progress_paused_ms`, `governed_task_revisions`, `governed_task_provenance`, `governed_task_proposals`, and `governed_task_steps` with expiry indexes and uniqueness/check constraints through the idempotent initializer in `atmem/store/sqlite.py` (FR-001, FR-002, FR-010, FR-026, FR-030, FR-036).
