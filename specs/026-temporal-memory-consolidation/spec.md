...
Target the complete native single-owner Memory Health profile for 2.4.0: temporal contracts, deterministic evaluation, review projection and actions, and an opt-in bounded background cycle with optional AtBot proposals. This local profile uses existing local Spec 006/015 authority and applicable evidence/access gates; it makes no shared-space or external-provider propagation claim. AtMem 2.3.4b2 installs the pinned AtFlows Python package by default, but its read-only evidence-handoff review leads remain opt-in and require a separately configured local AtFlows server. Those leads may feed the 2.4.0 health queue but cannot be required for health review; deterministic health checks must work with no AtFlows server running. Extend membership-safe review to shared spaces after the proposed 2.5.0 Spec 012/013 foundation, provider-aware review after 2.6.0 Spec 019/025 connections, investigation links after 2.7.0 Spec 020/021 work, and verified cross-provider invalidation/propagation after 2.8.0 Spec 023. These are staged release profiles, not claims that any version exists.
