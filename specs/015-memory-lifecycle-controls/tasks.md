...
- [ ] [T002] Add backward-compatible canonical migration and rollback in `atmem/store/sqlite.py` using the Spec 010-owned global migration sequence from `specs/integration-ownership.md`; test real persisted fixtures from every supported published AtMem upgrade floor in `tests/fixtures/upgrades/` and `tests/test_lifecycle_migration.py` (FR-009–FR-010)
...
- [ ] [T006] [US2] Implement forget verification including declared backup retention/crypto-erasure evidence in `atmem/lifecycle/maintenance.py`, register it through Spec 007's shared maintenance interface, and test it in `tests/test_forget_verification.py` (FR-007)
