...
official verification. The Hermes preview targets **2.3.8b3**;
...
The [2.3.4 AtFlows scope](../docs/release-roadmap.md#234-stable-scope) installs pinned AtFlows 0.1.1 with AtMem by default and offers optional AtMem-owned dashboard login and an opt-in read-only local evidence review report before the 2.4.0 Memory Health queue exists. AtFlows remains an independent observability package and supplies exact session-linked trace-error reasons only when configured and running; AtMem authorizes exact evidence and reports gaps. The versioned handoff may feed the 2.4.0 queue later. No automatic AtFlows start, raw-trace import, inferred link, automatic admission or causal conclusion from observational telemetry is implied. The handoff requires its 020/028 evidence gates on installed artifacts.
