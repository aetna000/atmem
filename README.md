...
[![Version 2.2.7b1](https://img.shields.io/badge/version-2.2.7b1-blue)](./docs/releases/v2.2.7b1.md)
...
```bash
python -m pip install 'atmem[pydantic-ai]==2.2.7b1'
...
```bash
python -m pip install 'atmem[langgraph]==2.2.7b1'
atmem control shadow --host generic --memory-db ~/.atmem/memories.db
```
...
This delegated host path is packaged in 2.2.7b1. The adapters do not replace
...
AtMem 2.2.7b1 includes Governed Task State: a separate, revisioned authority
...
the [2.2.7b1 release notes](docs/releases/v2.2.7b1.md) before upgrading.
...
> **Release status:** this repository describes the **AtMem 2.2.7b1 beta**, not verified publication. AtBot is a
...
AtMem uses its own governed retrieval by default. The 2.2.7b1 beta adds an
...
```bash
python -m pip install atmem==2.2.7b1
atmem --version
```
...
```bash
python -m pip install 'atmem[semantic]==2.2.7b1'
```
...
Current repository metadata is version **2.2.7b1**, with the matched OpenClaw
bridge **2.2.7-beta.1**. Release validation requires exact Python/bridge alignment;
AtBot retains its independent compatible version **0.1.0**.
