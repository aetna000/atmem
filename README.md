...
[![Version 2.3.2](https://img.shields.io/badge/version-2.3.2-blue)](./docs/releases/v2.3.2.md)
...
> **Release status:** AtMem 2.3.2 is the stable encrypted Agent Black Box release.
...
```bash
python -m pip install 'atmem[pydantic-ai]==2.3.2'
...
```bash
python -m pip install 'atmem[langgraph]==2.3.2'
atmem control shadow --host generic --memory-db ~/.atmem/memories.db
```
...
the [2.3.2 release notes](docs/releases/v2.3.2.md) before upgrading.
...
```bash
python -m pip install 'atmem[semantic]==2.3.2'
```
...
Current repository metadata is version **2.3.2**, with the matched OpenClaw
bridge **2.3.2**. Release validation requires exact Python/bridge alignment;
