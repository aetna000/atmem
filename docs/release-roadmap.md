...
**Status**: 2.3.5 stable release plus the 2.3.6b1 memory-integrity qualification; future versions are
planning targets, not delivery-date commitments.
**Stable release baseline**: AtMem `2.3.5`, OpenClaw bridge `2.3.5`, companion pins
...
| **2.3.6b1** — memory-integrity qualification | Canonical safety boundaries | Secret-bearing proposal refusal, direct-parent taint propagation, and issued scoped procedure-review authority; frozen before/after public benchmark evidence | Benchmarking 001 and existing memory/evidence authority contracts | Prerelease only. No schema migration; retrieval defaults and companion pins unchanged. Stable consideration requires tagged-artifact gates and the full rerun. |
