...
**Status**: 2.3.2 baseline plus proposed future sequence; future versions are
planning targets, not delivery-date commitments.
**Working baseline**: AtMem `2.3.2`, OpenClaw bridge `2.3.2`, companion pin
`atmem-atbot==0.1.0`.
...
Use prerelease candidates as needed for future scope. Stable AtMem `2.3.2` maps
exactly to bridge `2.3.2`. AtBot has an independent version and changes only when
