...
AtFlows is installed with AtMem 2.3.4b2 but its server and review are opt-in.
Start and initialize AtFlows separately before reviewing a run. The command
requires an authorized AtMem evidence token from `ATMEM_EVIDENCE_TOKEN` or an
interactive prompt and an AtFlows Administrator password from
`ATFLOWS_ADMIN_PASSWORD` or its prompt. It prints a versioned JSON report by
...
[2.3.4b2 release note](../releases/v2.3.4b2.md) for limits and setup.

## Upgrade and recovery
Follow the [published b1 release commands](../releases/v2.3.4b1.md), or the
[b2 candidate commands](../releases/v2.3.4b2.md) when testing that build, and
