...
async function registerContextAuthority(){var button=$("authorityRegister"),body={provider_id:$("authorityProvider").value.trim(),provider_version:$("authorityVersion").value.trim(),provider_instance_id:$("authorityInstance").value.trim(),key_id:$("authorityKeyId").value.trim(),public_key_base64:$("authorityPublicKey").value.trim(),endpoint:$("authorityEndpoint").value.trim(),workspace_ids:commaIds("authorityWorkspaces"),agent_ids:commaIds("authorityAgents"),user_ids:commaIds("authorityUsers"),native_fallback_on_failure:$("authorityFallback").checked};button.disabled=true;try{var result=await post("/api/delegated/register",body);$("authorityPublicKey").value="";renderContextAuthority(result.status);text("contextAuthorityGuidance","Provider registered but still disabled. Review the exact scopes, then press Enable.")}catch(error){showError(error)}finally{button.disabled=false}}
