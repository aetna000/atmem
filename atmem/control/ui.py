...
<title>AtMem memory and agent oversight</title>
<style>
:root{
...
.agentoverview{margin-bottom:18px}.agentoverviewhead{display:flex;gap:14px;align-items:start;margin-bottom:14px}
.agentoverviewhead>div:first-child{flex:1}.agentoverviewhead h2{margin:0 0 3px}.agentcounts{display:flex;gap:8px;flex-wrap:wrap}
.agentcount{min-width:104px;border:1px solid var(--line);border-radius:11px;background:var(--bg);padding:10px 12px}
.agentcount b{display:block;font-size:22px}.agentcount span{display:block;color:var(--muted);font-size:11px}
.agentstatus{display:flex;gap:9px;align-items:center;border-radius:10px;padding:11px 13px;margin-bottom:10px;background:var(--good-soft);border:1px solid color-mix(in srgb,var(--good) 38%,var(--line))}
.agentstatus.bad{background:var(--warn-soft);border-color:color-mix(in srgb,var(--warn) 45%,var(--line))}.agentstatus b{display:block}.agentstatus span{display:block;color:var(--muted);font-size:12px}
.agentmap{display:grid;gap:7px}.agentrow{display:grid;grid-template-columns:minmax(120px,.55fr) minmax(190px,.9fr) minmax(220px,1.25fr) auto;gap:12px;align-items:center;width:100%;text-align:left;border:1px solid var(--line);border-radius:10px;background:var(--card);color:var(--ink);padding:11px 13px}
.agentrow:hover{border-color:var(--brand);background:var(--brand-soft)}.agentrow b{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.agentrow small{display:block;color:var(--muted);overflow-wrap:anywhere}.agentscope b{color:var(--blue)}
...
.sectionnumber,.tabbtn>span,.storynumber{display:none}
.pageheading p,.pagecontext{display:none}.pageheading{margin-bottom:14px}.pageheading h1{margin-bottom:0}
.storystep{grid-template-columns:minmax(0,1fr);padding:12px 0;border-width:0 0 1px;border-radius:0;background:transparent}
.storystep:last-child{border-bottom:0}.storyflow{gap:0}
.activityhead{display:flex;gap:14px;align-items:center;justify-content:space-between;margin-bottom:14px}.activityhead h2{font-size:22px}.activitytools{display:grid;grid-template-columns:minmax(220px,1fr) auto auto auto;gap:8px;flex:1;max-width:820px}.activitytools input,.activitytools select{min-width:0;border:1px solid var(--line);border-radius:10px;background:var(--bg);color:var(--ink);padding:10px 12px}.activitytimeline{position:relative;display:grid;gap:0}.activitytimeline:before{content:"";position:absolute;left:15px;top:16px;bottom:16px;width:2px;background:var(--line)}.activityrow{position:relative;display:grid;grid-template-columns:32px minmax(0,1fr) auto;gap:13px;align-items:center;width:100%;padding:12px 8px;border:0;border-radius:11px;background:transparent;color:var(--ink);text-align:left}.activityrow:hover{background:var(--bg)}.activitynode{z-index:1;display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:var(--good);color:#fff;border:4px solid var(--card);font-weight:900}.activityrow.warn .activitynode{background:var(--warn)}.activityrow.bad .activitynode{background:var(--bad)}.activitycopy{min-width:0}.activitycopy b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.activitycopy span{display:block;margin-top:2px;color:var(--muted);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.activitytime{color:var(--muted);font-size:11px;white-space:nowrap}.activityempty{padding:18px 0;color:var(--muted)}.activitymore{display:flex;align-items:center;justify-content:center;gap:10px;margin-top:14px}.activitymore span{color:var(--muted);font-size:11px}
...
.activityhead{display:block}.activityhead h2{margin-bottom:12px}.activitytools{grid-template-columns:1fr 1fr}.activitytools input{grid-column:1/-1}.activitytime{display:none}
...
@media(max-width:780px){.attentionchecks{grid-template-columns:1fr}.attentionitem{grid-template-columns:1fr}.attentionhero{align-items:start}.agentoverviewhead{display:block}.agentcounts{margin:12px 0}.agentrow{grid-template-columns:1fr}.agentrow b,.agentrow span{white-space:normal}.agentoverviewhead>button{width:100%}}
</style>
</head>
<body>
<header><div class="wrap head">
  <div class="logo"><img class="brandmark" src="/assets/atmem.jpg" alt="AtMem logo"><strong>AtMem</strong></div>
  <div class="small" id="adapterLabel">Memory &amp; agent oversight</div>
  <div class="grow"></div>
...
<main class="wrap">
  ...
  <div class="pageheading"><div><div class="eyebrow">Live oversight</div><h1>Activity</h1></div></div>
  ...
  <section class="card agentoverview" id="agentOverview">
    <div class="agentoverviewhead"><div><div class="eyebrow">Persistent agent coverage</div><h2>Agents &amp; workspaces</h2><p class="sub">Which agents share memory, which are isolated, and whether every workspace is verified.</p></div><div class="agentcounts"><div class="agentcount"><b id="agentCount">—</b><span>agents detected</span></div><div class="agentcount"><b id="workspaceCount">—</b><span>workspaces</span></div></div><button class="secondary" id="agentRefresh" type="button">Refresh status</button></div>
    <div class="agentstatus" id="agentCoverageStatus"><span class="statusicon" id="agentCoverageIcon">…</span><div><b id="agentCoverageHeadline">Checking agent coverage…</b><span id="agentCoverageDetail">Reading the runtime’s persistent-agent topology.</span></div></div>
    <div class="agentmap" id="agentMap"><div class="activityempty">Loading agents…</div></div>
  </section>

  <section class="card" id="blackboxCard">
    <div class="activityhead"><h2>Recent actions</h2><div class="activitytools"><input id="activityQuery" aria-label="Search recent actions" placeholder="Search actions…"><select id="activityTopic" aria-label="Filter by topic"><option value="all">All topics</option><option value="agents">Agents</option><option value="memory">Memory</option><option value="tools">Tools</option><option value="web">Web</option><option value="email">Email</option><option value="conversation">Conversation</option></select><select id="activityWhen" aria-label="Filter by time"><option value="all">All time</option><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select><button class="secondary" id="blackboxRefresh" type="button">Refresh</button></div></div>
    <div class="activitytimeline" id="activityTimeline"><div class="activityempty">Loading…</div></div>
    <div class="activitymore"><button class="secondary" id="activityLoadMore" type="button">Load more</button><span id="activityCount"></span></div>
    <div class="actions" style="margin-top:14px"><button class="secondary" id="showAllFlights" type="button">Advanced search</button><button class="primary" id="bridgeRefresh" type="button" style="display:none">Upgrade bridge &amp; run test</button></div>
    <div id="attentionHero" hidden><b id="attentionHeadline"></b><p id="attentionSummary"></p></div><div id="attentionChecks" hidden></div><div id="attentionList" hidden></div>
  </section>

  <div class="sectiondivider"><span>Memory governance</span><p>Approve new memory and control whether AtMem may influence model context.</p></div>
  ...
  <section class="hero providerhero" id="hero">
    <div>
      ...
      <p id="summary">AtMem is verifying memory, evidence, and the safe return path.</p>
    </div>
    <div class="actions">
      <button class="secondary" id="refreshBtn">Refresh memory</button>
  ...
  <div class="grid">
    ...
    <aside>
      <section class="card">
        ...
        <p class="sub" id="switchCopy">AtMem checks the active adapter before allowing context injection.</p>
        <div id="checks"></div>
      ...
      <section class="card">
        <h2>What changes</h2>
        <div class="check"><i>✓</i><div>Shadow mode observes without adding memory to model context.</div></div>
        <div class="check"><i>✓</i><div>Active mode permits only context explicitly returned with <span class="mono">inject=true</span>.</div></div>
        <div class="check"><i>✓</i><div>Returning to shadow stops future injection and preserves evidence.</div></div>
        <div class="check"><i>✓</i><div>The runtime remains responsible for identity, tool execution and external outcome proof.</div></div>
      </section>
  ...
  <nav class="sectionnav" aria-label="Investigation sections"><a href="#blackboxArchiveCard"><span>01</span> Flights</a><a href="#memorySearchCard"><span>02</span> Memory search</a><a href="#mirrorCard"><span>03</span> Memory sources</a><a href="#auditExplorer"><span>04</span> Event evidence</a></nav>
  <div class="metrics">
    <div class="metric"><b class="mono" id="sourceCount">—</b><span id="sourceCountLabel">memory sources</span></div>
    <div class="metric"><b class="mono" id="recordCount">—</b><span>searchable memories</span></div>
    <div class="metric"><b class="mono" id="sourceBytes">—</b><span id="sourceBytesLabel">source bytes preserved</span></div>
    <div class="metric"><b id="verified">—</b><span>memory verification</span></div>
  </div>
  ...
  <section class="card" id="memorySearchCard">
    ...
    <p class="sub">Search the canonical AtMem view using ordinary words. This works in shadow and active mode.</p>
    <div class="search">
      <input id="query" placeholder="For example: TypeScript preference" autocomplete="off">
      <button class="primary" id="searchBtn">Search</button>
    </div>
    <div class="results" id="results"><div class="empty">Enter a question to inspect memory.</div></div>
  </section>

  <section class="card" id="mirrorCard">
    <div class="sectionnumber">03</div><h2 id="sourceTitle">Memory sources</h2>
    <p class="sub" id="sourceSummary">Loading source evidence…</p>
  ...
  <footer class="productfoot">
    ...
    <nav class="productlinks" aria-label="AtMem resources">
      ...
      <a href="https://github.com/aetna000/atmem/blob/main/docs/generic-adapter.md" target="_blank" rel="noopener noreferrer">Adapter guide</a><span>·</span>
    ...
    <div class="versionbar" aria-label="Installed product versions">
      ...
      <span class="versionchip" id="versionOpenClawChip">OpenClaw <b id="versionOpenClaw"><span class="zeros compact" aria-hidden="true"><span>0</span><span>0</span><span>0</span><span>0</span></span></b></span>
      ...
      <span class="versionchip" id="versionNpmChip">AtMem npm bridge <b id="versionNpm"><span class="zeros compact" aria-hidden="true"><span>0</span><span>0</span><span>0</span><span>0</span></span></b></span>
...
(function(){
"use strict";
var state=null,reviewQueue={records:[]},productInfo={},blackboxIndex={runs:[]},blackboxArchiveRows=[],blackboxStories={},flightRange="7d",bridgeRefreshStatus={available:false},attentionFilter="all",activityVisible=10,activitySearchTimer=null,csrf="",progressTimer=null,progressStarted=0;
...
function renderBlackbox(){
 ...
 var latest=currentRun(),groups=attentionGroups("all"),first=groups[0],hero=$("attentionHero"),hasCurrentIssue=!!first,reviewedLatest=!!(latest&&!(latest.attention_points||[]).length&&(latest.acknowledged_attention_points||[]).length);hero.className="attentionhero"+(hasCurrentIssue?" bad":"");
 ...
 else if(reviewedLatest){text("attentionHeadline","Reviewed — no action pending");text("attentionSummary","You acknowledged the latest finding. Its original evidence remains available in the flight history.")}
 ...
 var currentPoints=latest?(latest.attention_points||[]):[],completionBad=!latest||currentPoints.some(function(p){return p.check==="completion"}),toolsBad=currentPoints.some(function(p){return p.check==="tools"}),evidenceBad=!latest||currentPoints.some(function(p){return p.check==="context_model"});
 var checkRows=reviewedLatest?[[false,"Review complete","The original finding was acknowledged and remains in audit history."],[false,"Tool finding reviewed","No tool issue is waiting for action."],[false,"Evidence retained","Technical evidence and the acknowledgement remain available."]]:[[completionBad,"Flight completed",completionBad?"The latest activity did not close cleanly.":"Latest activity finished normally."],[toolsBad,"Tools worked",toolsBad?"A tool failed or could not be matched in the latest activity.":"No tool failure in the latest activity."],[evidenceBad,"Audit evidence complete",evidenceBad?"The latest activity is missing required evidence.":"Latest activity has complete audit evidence."]];
 checkRows.forEach(function(value){var item=element("div","attentioncheck "+(value[0]?"bad":"ok")),badge=element("span","checknumber",value[0]?"!":"✓"),copy=element("div");copy.append(element("b","",value[1]),element("span","",value[2]));item.append(badge,copy);checks.appendChild(item)});
 ...
 rows.forEach(function(row){var item=element("div","flight"),identity=element("div"),run=element("b","mono",row.run_id),session=element("div","small mono",(row.agent_id?"agent "+row.agent_id+" · ":"")+(row.session_id||"no session ID"));identity.append(run,session);
...
async function refreshReviews(){
 try{reviewQueue=await get("/api/memory/reviews");renderReviews()}catch(error){showError(error)}
}
...
function render(){
 ...
 text("stateChip",isActive?"AtMem active":needsRecovery?"Restore required":"Shadow mode");
 text("eyebrow",isActive?"Current memory provider":needsRecovery?"Interrupted switch detected":"Safe observation only");
 text("title",isActive?"AtMem memory is active for "+hostName:needsRecovery?"Restore "+hostName+" before activating":"AtMem is observing "+hostName+" in shadow mode");
...
$("activityQuery").addEventListener("input",function(){clearTimeout(activitySearchTimer);activitySearchTimer=setTimeout(function(){applyActivityFilters().catch(showError)},300)});$("activityTopic").onchange=function(){applyActivityFilters().catch(showError)};$("activityWhen").onchange=function(){applyActivityFilters().catch(showError)};$("activityLoadMore").onclick=function(){loadMoreActivity().catch(showError)};$("showAllFlights").onclick=function(){var query=$("activityQuery").value;showTab("Technical");$("flightQuery").value=query;filterBlackboxArchive();$("blackboxArchiveCard").scrollIntoView({behavior:"smooth",block:"start"})};
