...
async function inspectBlackbox(runId){
 ...
 try{var values=await Promise.all([get("/api/blackbox/flight?run_id="+encodeURIComponent(runId)),get("/api/blackbox/story?run_id="+encodeURIComponent(runId)).catch(function(){return{}})]),report=values[0],story=values[1]||{},body=$("auditorBody"),latest=currentRun();body.replaceChildren();var matrix=report.coverage_matrix||{},overall=matrix.overall_status||"unknown",ok=overall==="covered",operatorReview=report.operator_review||{},reportPoints=operatorReview.active_attention_points||report.attention_points||[],acknowledgedPoints=operatorReview.acknowledged_attention_points||[],historical=!!(latest&&latest.run_id!==runId),tools=report.tools||{},coverage=report.coverage||{},components=matrix.components||{},context=report.context||{};
  ...
  else if(reportPoints.length){decision.className="historicalnote attentionitem high";decisionTitle="Action required now: "+reportPoints[0].title;decisionCopy=reportPoints[0].detail+" Do this: "+reportPoints[0].action}
  ...
  if(!historical&&reportPoints.length){var actionCard=element("section","card"),actionBody=element("div","attentionlist");actionCard.append(element("h2","","What happened and what to do"),element("p","sub","Each finding names the exact observed boundary. Acknowledge it after you have reviewed the evidence; this removes it from the active queue without deleting history."));reportPoints.forEach(function(point){var item=element("div","attentionitem "+point.severity),copy=element("div","attentioncopy"),actions=element("div","reviewactions"),inspect=element("button","secondary","Open matching events"),ack=element("button","primary","Acknowledge and move on"),toolEvidence=[].concat(tools.missing_completions||[],tools.orphan_completions||[],tools.conflicting_requests||[],tools.conflicting_completions||[])[0];copy.append(element("b","",point.title),element("p","",point.detail),element("p","attentionaction","Do this: "+point.action));inspect.type="button";inspect.onclick=function(){focusFlightEvidence(toolEvidence||point.code)};ack.type="button";ack.onclick=function(){acknowledgeAttention(runId,point.code)};actions.append(inspect,ack);item.append(copy,actions);actionBody.appendChild(item)});actionCard.appendChild(actionBody);body.appendChild(actionCard)}
